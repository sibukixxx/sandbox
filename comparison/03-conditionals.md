...
1. **「式か文か」は決着済み**。7 言語すべてで `if` は式。Dafny だけが `method` 内では文になる (手続きと純粋関数を分けるため)。
...
4. **Option を組込みで持つか**。Rust / MoonBit / Lean / OCaml は組込み、Quint / Dafny は和型で自作、Verse は `?int` と `<decides>` の 2 通り。
